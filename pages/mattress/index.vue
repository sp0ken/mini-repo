  ...
  <main>
    <section class="columns product-choice mt-5">
      <h1 class="gill-regular text-uppercase text-center h3 show-md my-2 col-12 column lp-25">
        {{ $t('mattress.product-display.title.incredible-mattress') }}
      </h1>
        ...
        <div class="panel-body my-2">
          <no-ssr>
            <p
              v-if="$i18n.locale === 'fr'"
              class="mb-0"
              v-html="$t('mattress.product-display.text.incredible-mattress')"
            ></p>
            <div v-if="$i18n.locale === 'fr'" class="popover popover-bottom c-help">
              <p class="text-bold" v-html="$t('mattress.product-display.text.choice')"></p>

              <div class="popover-container">
                <div class="card bg-light">
                  <div
                    class="card-body"
                    v-html="$t('mattress.product-display.text.choice-details')"
                  ></div>
                  <cld-image
                    secure="true"
            ...
            <div v-else>
              ...
              <template v-if="$i18n.locale === 'fr'">
                <div class="h5 addons">{{ $t('mattress.product-display.title.addons') }}</div>
                <ted-checkbox :checked.sync="hasMattressReturn" name="mattressReturn">
                  {{
                    $t('mattress.product-display.label.mattress-return', {
                      returnPrice: $options.returnPrice
                    })
                  }}
                  <span
                    class="tooltip text-warning"
                    :data-tooltip="$t('mattress.product-display.tooltip.product-return-terms')"
                    >ⓘ</span
                  >
                </ted-checkbox>
                <ted-checkbox
                ...
                >
                  {{
                    $t('mattress.product-display.label.mattress-protector', {
                      addonPrice: addons['mattress-protector'].price
                    })
                  }}
                </ted-checkbox>
                <ted-checkbox
                  v-if="addons['bed-head'].code"
                  :checked.sync="addons['bed-head'].selected"
                  name="bedHead"
                >
                  {{
                    $t('mattress.product-display.label.bed-head', {
                      addonPrice: addons['bed-head'].price
                    })
                  }}
                </ted-checkbox>
              </template>
          ...
          >
            <div v-show="selectedProductPrice > 0" class="product-price my-0 py-1">
              ...
              <small
                class="tooltip"
                :data-tooltip="
                  $t('mattress.product-display.text.eco-tax', {
              ...
              <small
                ...
                v-html="
                  $t('mattress.product-display.text.price-monthly', {
            ...
            <div v-show="product && selectedProductPrice === 0" class="product-price my-0 py-1">
              <span class="h3">{{ $t('mattress.product-display.text.out-of-stock') }}</span>
            </div>
          </div>
          ...
          <no-ssr>
            <div v-if="product" class="col-12 column text-center">
              <button
                :title="$t('mattress.product-display.button.add-to-cart')"
              ...
              >
                ...
                <span v-if="!$wait.waiting('cart.add')">{{
                  $t('mattress.product-display.button.add-to-cart')
                }}</span>
              </button>
            </div>
          </no-ssr>
    ...
    <section class="product-story p-relative margin-top-simple">
      <ted-story class="col-9 col-md-12 col-mx-auto column mt-10">
        <template #excerpt>
          <span v-html="$t('mattress.story.text.excerpt')"></span>
        </template>
        ...
        <template #image>
          <cld-image
            ...
            lazy
            placeholder="lqip"
            :alt="$t('mattress.story.alt.story')"
          >
            <cld-transformation
              width="auto:120:300"
              crop="scale"
        ...
        <template #body>
          <span v-html="$t('mattress.story.text.body')"></span>
        </template>
      </ted-story>
        ...
        script: [
          {
            ...
            json: {
              ...
              '@type': 'Product',
              'name': this.$t('mattress.meta.title'),
              'image': [
                'https://res.cloudinary.com/dgodjz1pn/image/upload/c_scale,dpr_auto,q_auto,f_auto,w_auto:120:600/v1588595042/products/mattress/slider/slide-5-square.jpg',
                'https://res.cloudinary.com/dgodjz1pn/image/upload/c_scale,dpr_auto,q_auto,f_auto,w_auto:120:1200/v1588595042/products/mattress/slider/slide-5.jpg'
              ],
              'description': this.$t('mattress.meta.description'),
              'brand': 'Tediber',
              'gtin': '3760292711240',
              'aggregateRating': {
          ...
          },
          {
            ...
            json: {
              '@context': 'http://schema.org',
              '@type': 'VideoObject',
              'name': this.$t('mattress.meta.video-name'),
              'description': this.$t('mattress.meta.video-description'),
              'thumbnailUrl': [
                `https://i.ytimg.com/vi/${this.$t('mattress.meta.video-id')}/sddefault.jpg`,
                `https://i.ytimg.com/vi/${this.$t('mattress.meta.video-id')}/mqdefault.jpg`,
                `https://i.ytimg.com/vi/${this.$t('mattress.meta.video-id')}/hqdefault.jpg`
              ],
              'uploadDate': this.$t('mattress.meta.video-upload-date'),
              'duration': this.$t('mattress.meta.video-duration'),
              'contentUrl': `https://www.youtube.com/watch?v=${this.$t('mattress.meta.video-id')}`,
              'embedUrl': `https://www.youtube.com/embed/${this.$t('mattress.meta.video-id')}`
            }
          }
        ]
      }
